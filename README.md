...
- 🖥️ **Web 可视化控制台**：告别枯燥的命令行，全程在网页上点点点。
- 🕵️ **人工精细审核**：双面板设计，左侧看“媒体（图片/视频）”，右侧看“情报（聊天记录）”。
- 🛡️ **多级防封号策略**：
  - **智能分流**：根据选中数量自动切换“逐条直发”与“合并打包”。
  - **三级兜底**：打包失败时自动降级尝试（克隆节点 -> 引用节点），最大程度防风控。
  - **随机延迟**：模拟真人发送速度，避免被判定为机器人刷屏。
- 📨 **管理员私信预览**：不敢确定的聊天记录？一键私发给审核员 QQ 预览。
...
### 第一步：配置 NapCatQQ
打开你的 NapCat 网页控制台（WebUI），进入**网络配置**，开启 **正向 WebSocket**，并将端口设置为 `3001`。
*(如果你不知道怎么设，只要确保 NapCat 监听了 `ws://127.0.0.1:3001` 即可)*

### 第二步：下载本项目
将本项目的代码文件夹下载到你的电脑上，并解压。
...
### 2. 开始运行
- 点击左上角绿色的 【开始接客】 按钮。如果右侧的 STATUS 亮起绿灯 🟢，说明已成功连接到 QQ 机器人！此时如果“来源群”有人发图片、视频或合并转发记录，就会自动出现在你的审核面板上。
...
### ❓ 常见问题 (FAQ)
- Q1：为什么点击“开始接客”后，状态灯是黄色的 🟡 或红色的 🔴？并且日志一直提示“连接断开”？
